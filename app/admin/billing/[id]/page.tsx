...
export default async function InvoiceDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const detail: InvoiceDetail = {
    ...
    lineItems: invoice.lineItems.map((li) => ({
      ...
      amount: li.amount ?? 0,
      cdrIdentifier: li.cdrIdentifier ?? "",
      wholesaleAmount: li.wholesaleAmount ?? null,
      retailPlanName: li.retailPlanName ?? "",
      pricingMethod: li.pricingMethod ?? "",
      markupPercent: li.markupPercent ?? null,
      fixedPrice: li.fixedPrice ?? null,
    })),
